<?php include "header.tpl.php"; ?>

    <div id="details">
        <p>
            <strong>Doe mee(r) met Speeltuinzoeker!</strong><br>
            Deze website is voor en door de gebruikers zelf. We helpen elkaar door speeltuinen aan te maken, zodat de site altijd zo volledig en actueel mogelijk is.<br>
            Maak een <a href="admin/register.php">account</a> aan en krijg toegang tot Mijn Speeltuinzoeker.<br>
            Daar kun je zelf speeltuinen aanmaken, bewerken en foto's toevoegen.
            En in de toekomst kun je, als ingelogde gebruiker, ook andere speeltuinen beoordelen.<br>
            Maak vandaag nog snel en eenvoudig je <a href="admin/register.php">account</a> aan!
        </p>
    </div>

<?php include "footer.tpl.php"; ?>

</html>