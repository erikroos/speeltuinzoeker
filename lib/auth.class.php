...
class Auth {
	...
	public function login($user_unesc, $pass_unesc, $salt) {
		$login = $this->db->realEscapeString($user_unesc);
		$password = $this->hashPassword($pass_unesc, $salt);
	...
	public function createNewAccount($name, $email, $password, $salt) {
		...
		$this->db->query(sprintf("INSERT INTO user 
				...
				VALUES (\"%s\", \"%s\", \"%s\", 0, 0, \"%s\");",
				$this->db->realEscapeString($name), $this->db->realEscapeString($email), 
				$this->hashPassword($password, $salt), $activationCode));
		...
		$message = "<p>Beste " . $name . ",</p>" .
				...
				"<a href='" . $baseUrl . "admin/activate.php?code=" . $activationCode . "'>deze link</a>" . 
				...
				"<p>PS: werkt de link niet? Voer dan dit webadres in in de adresbalk van je browser:<br>" . 
				$baseUrl . "admin/activate.php?code=" . $activationCode . "</p>";
		
		Mail::sendMail($email, "Activeer je aanmelding bij Speeltuinzoeker.nl", $message);
	}
	
	public function activateAccount($code) {
		$res = $this->db->query(sprintf("SELECT id FROM user WHERE active = 0 AND activation_code = \"%s\"", 
			$this->db->realEscapeString($code)));
		if ($row = $res->fetch_assoc()) {
